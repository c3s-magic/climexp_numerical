...
def RunProcess(cmds, callback=None, env=None, bufsize=2):
    """
    Runs a process, calls callback with stdout/stderr during execution,
    and returns the exit code when finished
    """
    try:
    ...
    p = Popen(cmds, universal_newlines=True,
              stdout=PIPE, stdin=PIPE, stderr=PIPE,
              bufsize=bufsize, close_fds=ON_POSIX,
              env=env, preexec_fn=setlimits)
    ...
    for i in range(2):
        ...
        while True:
            try:
                ...
                if(stdoutThread.isAlive() is False):
                    break
